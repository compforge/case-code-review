...
func TestResolveLineNumbers_SingleLineHunkMatch(t *testing.T) {
	...
	// Wait — need to trace carefully: ctx line is change.HunkContext, offset++ makes it 1 before next iteration.
...
func TestExtractSideLines_NewSide(t *testing.T) {
	hunk := change.Hunk{
		...
		Lines: []change.HunkLine{
			{change.HunkContext, "    ctx := r.Context()"},
			{change.HunkDeleted, `    log.Print("old")`},
			{change.HunkAdded, `    log.Printf("new: %s", r.URL)`},
			{change.HunkContext, "    err := process(ctx)"},
		},
	}
...
func TestExtractSideLines_DivergentStartLines(t *testing.T) {
	hunk := change.Hunk{
		...
		Lines: []change.HunkLine{
			{change.HunkContext, "A"},
			{change.HunkAdded, "B"},
			{change.HunkContext, "C"},
		},
	}
...
func TestExtractSideLines_OnlyAdded(t *testing.T) {
	hunk := change.Hunk{
		...
		Lines: []change.HunkLine{
			{change.HunkAdded, "line1"},
			{change.HunkAdded, "line2"},
		},
	}
...
func TestExtractSideLines_OnlyDeleted(t *testing.T) {
	hunk := change.Hunk{
		...
		Lines: []change.HunkLine{
			{change.HunkDeleted, "old1"},
			{change.HunkDeleted, "old2"},
		},
	}
...
+    y := 2
	...
	diffs := []change.Change{{NewPath: "test.go", Diff: raw}}
	comments := []Finding{
		{Path: "test.go", ExistingCode: `    x := 1
    y := 2`},
	}
...
func TestResolveLineNumbers_AlreadyResolved(t *testing.T) {
	diffs := []change.Change{{NewPath: "test.go", Diff: testDiff}}
	comments := []Finding{
...
+func newFunc() {}`
	...
	diffs := []change.Change{{OldPath: "old_name.go", NewPath: "new_name.go", Diff: raw}}
