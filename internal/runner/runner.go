...
func New(args Args) *Runner {
	...
	// diff.Diff records lazily (a.diffs is only populated by loadDiffs,
	// after New returns).
...
// Diffs returns the parsed diffs loaded by the runner.
func (a *Runner) Diffs() []diff.Diff {
	return a.diffs
}
...
func (a *Runner) loadDiffs(ctx context.Context) error {
...
func (a *Runner) runReviewFilters(ctx context.Context) {
...
func (a *Runner) executeReviewFilter(ctx context.Context, d diff.Diff) {
...
func buildFilterCommentsJSON(comments []finding.Finding) string {
...
func (a *Runner) filterLargeDiffs(diffs []diff.Diff) []diff.Diff {
...
func (a *Runner) countReviewable(diffs []diff.Diff) int {
	count := 0
	for _, d := range diffs {
		if !a.shouldReview(d) {
			continue
...
func (a *Runner) shouldReview(d diff.Diff) bool {
...
func (a *Runner) filterDiffs(diffs []diff.Diff) []diff.Diff {
	var kept []diff.Diff
	skipped := 0
...
func (a *Runner) findDiff(path string) *diff.Diff {
	for i := range a.diffs {
