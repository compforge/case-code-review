...
func ResolveLineNumbers(comments []Finding, diffs []diff.Diff) []Finding {
	if len(comments) == 0 || len(diffs) == 0 {
		return comments
	}

	// Build lookup: newPath -> *Diff
	diffByPath := make(map[string]*diff.Diff, len(diffs))
	for i := range diffs {
		d := &diffs[i]
...
func ResolveComment(cm *Finding, d *diff.Diff) bool {
	if cm.StartLine > 0 || cm.EndLine > 0 {
		return true
	}
	if cm.ExistingCode == "" {
...
func resolveFromHunk(d *diff.Diff, cm *Finding) bool {
...
func resolveFromFileContent(d *diff.Diff, cm *Finding) bool {
	if d.NewFileContent == "" {
		return false
	}
