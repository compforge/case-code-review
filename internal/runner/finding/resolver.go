...
func ResolveLineNumbers(comments []Finding, diffs []change.Change) []Finding {
	if len(comments) == 0 || len(diffs) == 0 {
		return comments
	}

	// Build lookup: newPath -> *Diff
	diffByPath := make(map[string]*change.Change, len(diffs))
	for i := range diffs {
		d := &diffs[i]
	...
	result := make([]Finding, len(comments))
...
func resolveFromHunk(d *change.Change, cm *Finding) bool {
...
func extractSideLines(hunk *change.Hunk, newSide bool) []indexedLine {
...
func resolveFromFileContent(d *change.Change, cm *Finding) bool {
	if d.NewFileContent == "" {
		return false
	}
