// Package llmloop carries the per-file LLM tool-use loop shared by `ccr
// review` (diff-based) and `ccr scan` (full-file). It owns the chat
...
// Warning describes a non-fatal warning recorded during a per-file
// review/scan.
type Warning struct {
...
func (p *CommentWorkerPool) Submit(f func() ([]finding.Finding, error)) {
	p.wg.Go(func() {
		p.semaphore <- struct{}{}
		...
		comments, err := f()
		if err != nil {
			fmt.Fprintf(stdout.Writer(), "[ccr] CommentWorkerPool error: %v\n", err)
		}
		p.resultsMu.Lock()
...
func (p *CommentWorkerPool) Await() []finding.Finding {
	p.wg.Wait()
	return p.results
}
