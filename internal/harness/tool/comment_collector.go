...
func (c *CommentCollector) Add(cm finding.Finding) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *CommentCollector) Comments() []finding.Finding {
	c.mu.Lock()
	defer c.mu.Unlock()
	out := make([]finding.Finding, len(c.comments))
	copy(out, c.comments)
	return out
}

// CommentsForPath returns a copy of comments whose Path matches the given path.
func (c *CommentCollector) CommentsForPath(path string) []finding.Finding {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *CommentCollector) Since(start int) []finding.Finding {
	c.mu.Lock()
	defer c.mu.Unlock()
	if start < 0 {
		start = 0
	...
	out := make([]finding.Finding, len(c.comments)-start)
...
// are ignored (no-op).
func (c *CommentCollector) ReplaceSince(start int, replacements []finding.Finding) {
	c.mu.Lock()
	defer c.mu.Unlock()
	if start < 0 {
		start = 0
