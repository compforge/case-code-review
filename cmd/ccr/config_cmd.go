...
func runConfig(args []string) error {
	...
	case "provider":
		if len(args) != 1 {
			return fmt.Errorf("config provider does not accept arguments; use 'ccr config set provider <name>' for non-interactive setup")
		}
		return runConfigProvider()
	case "model":
		if len(args) != 1 {
			return fmt.Errorf("config model does not accept arguments; use 'ccr config set model <name>' for non-interactive setup")
		}
