...
**3. 把发现与裁决分开。** 每个 Unit Review 可以产出零到多个可证伪的 Hypothesis，不直接发布评论；相关 Hypothesis 流式归入多个有界 Dossier，由独立 Hypothesis Review 只读复核并增量提交 Assessment；确定性的 Trial 只交付证据支持、由本次变更造成、值得行动且不重复的 Finding。
