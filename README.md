...
**3. Separate discovery from decision.** Each Unit Review produces zero or more falsifiable Hypotheses, not public comments. Related Hypotheses stream into bounded Dossiers; independent Hypothesis Reviews verify them with read-only evidence and submit Assessments incrementally. Deterministic Trial rules publish only supported, actionable and non-duplicate Findings caused by the current change.
