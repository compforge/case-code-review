...
- `supported` means the trigger, execution path, diff attribution, and observable impact are all grounded in checked evidence.
...
- When several hypotheses describe the same underlying defect, keep one canonical hypothesis actionable and mark redundant deliveries low-value with the canonical hypothesis ID in the reason.

## Completion

Submit exactly one assessment for every supplied hypothesis with `submit_assessments`, then call `task_done`. The final Trial is deterministic: only `supported + actionable` can become a Finding.
