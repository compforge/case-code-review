...
func TestResolveLineNumbers_SingleLineHunkMatch(t *testing.T) {
	...
	// Wait — need to trace carefully: ctx line is diff.HunkContext, offset++ makes it 1 before next iteration.
...
func TestExtractSideLines_NewSide(t *testing.T) {
	hunk := diff.Hunk{
		...
		Lines: []diff.HunkLine{
			{diff.HunkContext, "    ctx := r.Context()"},
			{diff.HunkDeleted, `    log.Print("old")`},
			{diff.HunkAdded, `    log.Printf("new: %s", r.URL)`},
			{diff.HunkContext, "    err := process(ctx)"},
		},
	}
...
func TestExtractSideLines_OnlyAdded(t *testing.T) {
	hunk := diff.Hunk{
		...
		Lines: []diff.HunkLine{
			{diff.HunkAdded, "line1"},
			{diff.HunkAdded, "line2"},
		},
	}
...
func TestExtractSideLines_OnlyDeleted(t *testing.T) {
	hunk := diff.Hunk{
		...
		Lines: []diff.HunkLine{
			{diff.HunkDeleted, "old1"},
			{diff.HunkDeleted, "old2"},
		},
	}
...
+func newFunc() {}`
	...
	diffs := []diff.Diff{{OldPath: "old_name.go", NewPath: "new_name.go", Diff: raw}}
