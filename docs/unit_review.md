...
Clue / Unit == Unit Review ==> Hypothesis ==> Lane / Hypothesis Review
                                             ==> Assessment == Trial (Review 3) ==> Finding
```
...
空参数、损坏 JSON 或需要猜测业务意图的输入仍显式失败并进入轨迹统计。高频错误应优先反推工具
命名、schema 和示例，而不是让修复器无限扩张成另一套模糊协议。

参数校验失败时，错误反馈除指出缺失字段外，还应优先根据**本次失败参数**生成相关的最小合法
payload：保留其中已经合法且能标识业务目标的值，只补无歧义的结构、删除可省略的非法字段。例如
`read_files {"reads":[{"file_path":"a.py","start_line":20,"end_line":"bad"}]}` 可提示
`{"reads":[{"file_path":"a.py","start_line":20}]}`。若原参数是 `{}`，没有任何路径或 query 可复用，
则只能返回带占位符的 schema 示例，不能从 reasoning 猜业务参数。提示只帮助模型下一轮修正，不会
自动执行；原始失败仍保留在轨迹里。

Trajectory eval 还应持续按 tool **积累**常见非法参数的原始 payload 和代表性 case，不能只统计次数，
更不能把它们笼统归因于模型能力。多数不离谱的输入已经表达了业务目标，只是数组层级、字段名或类型
不符合契约；目标和值都明确时，应优先改进 schema affordance、返回与本次输入相关的修正示例，或在
Harness 做无损归一化。若同类输入反复表达一种现有参数无法覆盖的诉求，则说明是工具契约缺口，应
扩展或重塑参数，而不是要求模型继续猜。只有 payload 没有目标信息、语义冲突或必须猜业务意图时，
才保留为不可自动修正的非法调用。
