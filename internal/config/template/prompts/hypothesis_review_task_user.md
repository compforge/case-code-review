...
Use `file_read_diff` to inspect the complete diff for each path relevant to a hypothesis. Use `file_read` and `code_search` for surrounding implementation and call-path evidence.
